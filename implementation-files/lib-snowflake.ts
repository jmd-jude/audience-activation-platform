  ...
  private async connect(): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        // Parse and convert private key to DER format
        const crypto = require('crypto');
        let privateKeyDer: Buffer;
        
        try {
          // Convert PEM to DER format for Snowflake
          const privateKeyObj = crypto.createPrivateKey({
            key: privateKeyContent,
            format: 'pem'
          });
          
          privateKeyDer = privateKeyObj.export({
            format: 'der',
            type: 'pkcs8'
          });
        } catch (keyError) {
          reject(new Error(`Failed to process private key: ${keyError.message}`));
          return;
        }
